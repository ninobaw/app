...
function calculateTagRelevance(correspondanceContext, tag) {
  ...
  // 4. Bonus pour certains patterns spécifiques
  const bonusPatterns = {
    'urgent': /urgent|priorité|immédiat|critique|asap/i,
    'confidentiel': /confidentiel|secret|privé|classé/i,
    'technique': /technique|maintenance|réparation|équipement|système/i,
    'financier': /financier|finance|budget|coût|facture|paiement/i,
    'administratif': /administratif|administration|procédure|dossier/i,
    'formation': /formation|cours|apprentissage|stage|éducation/i,
    'réunion': /réunion|meeting|assemblée|conférence/i,
    'rh': /personnel|employé|recrutement|ressources.humaines|rh/i
  };
  
  const pattern = bonusPatterns[tag.name.toLowerCase()];
...
// Fonction principale d'assignation intelligente des tags
async function smartTagAssignment() {
  try {
    // Connexion à MongoDB
    await mongoose.connect(process.env.MONGODB_URI || 'mongodb://localhost:27017/aero-doc-flow');
    console.log('✅ Connexion à MongoDB établie');

    // Récupérer tous les tags actifs de la base de données
    const tags = await Tag.getActiveTags();
    console.log(`📋 Tags disponibles dans la base: ${tags.length}`);
    
    if (tags.length === 0) {
      console.log('⚠️  Aucun tag actif trouvé dans la base de données');
      console.log('💡 Exécutez d\'abord: node src/scripts/init-default-tags.js');
      process.exit(1);
    }
    ...
    for (const correspondance of correspondances) {
      try {
        ...
        // Calculer le score de pertinence pour chaque tag
        ...
        const minScore = 15; // Seuil minimum de pertinence
        const maxTags = 3; // Maximum 3 tags par correspondance
    ...
    console.log('\n📈 Résumé de l\'assignation intelligente:');
    ...
    if (updatedCount > 0) {
      console.log('\n🎉 Assignation intelligente terminée avec succès !');
      console.log('💡 Les correspondances ont maintenant des tags pertinents basés sur leur contenu');
    } else {
...
// Fonction de test pour analyser une correspondance spécifique
async function testSingleCorrespondance(correspondanceId) {
  try {
    await mongoose.connect(process.env.MONGODB_URI || 'mongodb://localhost:27017/aero-doc-flow');
    ...
    console.log(`🔍 Analyse de: "${correspondance.subject}"`);
    console.log(`📝 Contenu: "${context.fullText.substring(0, 200)}..."`);
    console.log(`🔑 Mots-clés: [${context.keywords.join(', ')}]`);
    ...
    console.log('\n📊 Scores de tous les tags:');
    tagScores.forEach(item => {
      console.log(`   ${item.tag.name}: ${item.score.toFixed(1)} points`);
    });
...
if (require.main === module) {
  ...
  if (testId) {
    console.log('🧪 TEST D\'UNE CORRESPONDANCE SPÉCIFIQUE\n');
    testSingleCorrespondance(testId);
  } else {
    console.log('🤖 ASSIGNATION INTELLIGENTE DES TAGS\n');
    smartTagAssignment();
  }
}
